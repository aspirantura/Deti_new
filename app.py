...
# ============================================================
# ПРЕДУПРЕЖДЕНИЕ О ЦЕЛЕВОЙ ГРУППЕ
# ============================================================
st.markdown("""
<div class="warning-box">
    ⚠️ <b>Внимание!</b> Данный калькулятор предназначен для <b>недоношенных детей</b> (срок гестации &lt; 37 недель), зачатых методом ЭКО.<br>
    Для доношенных детей модель не валидирована.
</div>
""", unsafe_allow_html=True)
...
st.markdown("""
<div class="info-box">
    ℹ️ Прогностический коэффициент за недоношенность (+2,41 пата) учитывается автоматически.
</div>
""", unsafe_allow_html=True)
...
with st.expander("ℹ️ Информация о модели"):
    st.markdown("""
    **Показатели диагностической эффективности модели** (для недоношенных детей):
    
    | Показатель | Значение | 95% ДИ |
    |------------|----------|--------|
    | AUC | 0,895 | 0,769–0,984 |
    | Чувствительность | 86,0% | 64,3–100,0% |
    | Специфичность | 70,3% | 54,3–86,1% |
    | NPV | 91,5% | 77,3–100,0% |
    
    **Метод:** прогностические коэффициенты рассчитаны по методу Вальда с использованием дивергенции Кульбака.
    
    **Валидация:** внутренняя валидация методом bootstrap (n=1000).
    """)
